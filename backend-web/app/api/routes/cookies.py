...
from common.utils.xianyu_utils import close_account_notice, trans_cookies
...
async def create_account(
    ...
    try:
        owner_id, _ = resolve_owner_scope(current_user)
        cookie_dict = trans_cookies(payload.value or "")
        cookie_unb = (cookie_dict.get("unb") or "").strip() or None

        # 手动输入 Cookie 时，如果账号ID或 Cookie 内 unb 已存在，则更新现有账号，避免重复新增。
        # 兼容旧用法：账号ID可以不填，直接从 Cookie 的 unb 自动匹配已有账号。
        payload_id = (payload.id or "").strip()
        account = None
        if payload_id:
            account = await account_service.get_account_for_user(owner_id, payload_id)
        if account is None and cookie_unb:
            account = await account_service.get_account_for_user(owner_id, cookie_unb)

        if account is None:
            return ApiResponse(
                success=False,
                message="账号不存在：手动填写Cookie只允许更新已有账号，不会新增账号。请确认Cookie里包含已有账号的unb，或填写账号列表里的准确账号ID。",
            )

        # 兼容旧镜像里的 AccountService.update_cookie(account, value) 签名；
        # 额外字段在路由层直接更新，避免手动更新 Cookie 报 unexpected keyword argument。
        await account_service.update_cookie(account, payload.value)
        if cookie_unb:
            account.unb = cookie_unb
        ...
        await account_service.session.refresh(account)
        
        # 启动WebSocket任务（通过HTTP调用WebSocket服务）
        from app.services.websocket_client import websocket_client
        task_result = await websocket_client.start_account(account.account_id, account.cookie or payload.value or "", account.owner_id)
        if isinstance(task_result, dict) and not task_result.get("success", True):
            return ApiResponse(success=False, message=task_result.get("message") or "账号Cookie已更新，但启动任务失败")
        
    except ValueError as exc:
        return ApiResponse(success=False, message=str(exc))
    except Exception as exc:
        return ApiResponse(success=False, message=f"更新账号Cookie失败: {str(exc)}")
    return ApiResponse(success=True, message="账号Cookie已更新")
...
async def update_account_cookie(
    ...
    cookie_unb = None
    try:
        cookie_unb = (trans_cookies(payload.value or "").get("unb") or "").strip() or None
    except Exception:
        cookie_unb = None
    # 兼容旧镜像里的 AccountService.update_cookie(account, value) 签名；
    # 额外字段在路由层直接更新。
