...
class CardMatcher:
    ...
    def _match_cards_by_spec(
        ...
        multi_spec_cards = [card for card in cards if card.is_multi_spec]
        value_only_matches: List[Dict[str, Any]] = []
        ...
        for card in cards:
            if card.is_multi_spec:
                if has_spec_info:
                    ...
                    elif card_sv == input_sv:
                        value_only_matches.append(self._card_to_dict(card))
                        logger.info(
                            f"多规格卡券规格值候选匹配: {card.name} "
                            f"卡券[{card.spec_name}:{card.spec_value}] vs 订单[{spec_name}:{spec_value}]"
                        )
                    else:
                        logger.debug(
                ...
                # 非多规格卡券：只有在没有传入规格信息且没有多规格卡券时才添加
                if not has_spec_info and not multi_spec_cards:
                    matched.append(self._card_to_dict(card))

        if not matched and len(value_only_matches) == 1:
            logger.info(
                f"多规格卡券按规格值唯一匹配成功: {value_only_matches[0].get('name')} "
                f"[{spec_name}:{spec_value}] -> [{value_only_matches[0].get('spec_name')}:{value_only_matches[0].get('spec_value')}]"
            )
            matched = value_only_matches
        elif not matched and len(value_only_matches) > 1:
            logger.warning(f"多规格卡券按规格值匹配到 {len(value_only_matches)} 张，无法唯一确定，跳过")
    ...
    def _match_card_dicts_by_spec(
        ...
        # 现场兼容：闲鱼订单规格名可能是“密钥”，商家卡券规格名可能统一填“规格”。
        # 在规格值唯一命中的情况下允许只按 spec_value 匹配，避免因规格名别名导致自动发货失败。
        multi_spec_cards = [cd for cd in card_dicts if cd.get("is_multi_spec")]
        value_only_matches: List[Dict[str, Any]] = []
            ...
            if cd.get("is_multi_spec"):
                if has_spec_info:
                    ...
                    if card_sn == input_sn and card_sv == input_sv:
                        ...
                        value_only_matches.append(cd)
                        logger.info(
                            f"多规格卡券规格值候选匹配: {cd.get('name')} "
                            f"卡券[{cd.get('spec_name')}:{cd.get('spec_value')}] vs 订单[{spec_name}:{spec_value}]"
                        )
                    else:
                        logger.debug(
