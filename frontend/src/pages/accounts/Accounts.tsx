...
export function Accounts() {
  ...
  const handleManualAdd = async (e: FormEvent) => {
    ...
    try {
      ...
      if (accountId) {
        ...
        if (!existingAccount) {
          addToast({ type: 'error', message: `账号 ${accountId} 不存在，请选择已有账号ID更新Cookie，不会新增账号` })
          return
        }
        result = await updateAccountCookie(accountId, cookieValue)
      } else {
        // 账号ID不填时，交给后端从 Cookie 中的 unb 识别已有账号并更新。
        // 后端已禁止新增：找不到已有账号会返回失败。
        result = await addAccount({ id: '', cookie: cookieValue })
      }

      // 后端返回 {msg: 'success'} 或 {success: true}
      if (result.success || result.msg === 'success') {
        addToast({ type: 'success', message: 'Cookie已更新' })
        closeModal()
        loadAccounts()
      } else {
        addToast({ type: 'error', message: result.message || result.detail || '更新失败' })
      }
    } catch (error: unknown) {
      // 获取后端返回的错误信息
      const axiosError = error as { response?: { data?: { detail?: string; message?: string } } }
      const errorMessage = axiosError.response?.data?.detail || axiosError.response?.data?.message || '保存Cookie失败'
  ...
  const handleSaveAISettings = async () => {
    ...
    try {
      ...
      addToast({ type: 'success', message: 'AI设置已保存，并已同步为全局默认配置' })
      closeModal()
      await loadAccounts()
    } catch (error) {
      ...
      {activeModal === 'manual' && (
        <div className="modal-overlay">
          <div className="modal-content max-w-md">
            <div className="modal-header">
              <h2 className="modal-title">手动更新Cookie</h2>
              <button onClick={closeModal} className="modal-close">
                <X className="w-4 h-4" />
              </button>
            </div>
            <form onSubmit={handleManualAdd}>
              <div className="modal-body space-y-4">
                <div className="input-group">
                  <label className="input-label">账号ID（可选）</label>
                  <input
                    type="text"
                    value={manualAccountId}
                    onChange={(e) => setManualAccountId(e.target.value)}
                    className="input-ios"
                    placeholder="可不填；不填时自动用Cookie里的unb匹配已有账号"
                    autoFocus
                  />
                </div>
                <div className="input-group">
                  ...
                  />
                  <p className="input-hint">
                    这里只更新已有账号的Cookie，不会新增账号。账号ID可不填；不填时会自动用Cookie里的unb匹配已有账号。
                  </p>
                </div>
              </div>
              <div className="modal-footer">
...
要发送给买家的内容`}</pre>
      ...
      {activeModal === 'ai-settings' && aiSettingsAccount && (
        <div className="modal-overlay">
          <div className="modal-content max-w-lg">
            <div className="modal-header">
              <h2 className="modal-title">AI回复设置（全局默认）</h2>
              <button onClick={closeModal} className="modal-close">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="modal-body space-y-4">
              {aiSettingsLoading ? (
                ...
                <>
                  <div className="input-group">
                    <label className="input-label">当前账号</label>
                    <input
                      type="text"
                    ...
                    <p className="text-xs text-blue-600 dark:text-blue-400 mt-1 leading-relaxed">
                      服务商、API Key、模型、提示词和议价参数保存后会作为全局默认配置，自动同步给所有账号；每个账号的“启用AI回复”开关仍然独立。
                    </p>
                  </div>
