...
# Redis缓存配置（Docker内置）
REDIS_PASSWORD=change-me-in-env
REDIS_DB=0
...
services:
  ...
  mysql:
    ...
    environment:
      - MYSQL_ROOT_PASSWORD=${MYSQL_ROOT_PASSWORD:-change-me-in-env}
      - MYSQL_DATABASE=${MYSQL_DATABASE:-xianyu_data}
      - MYSQL_USER=${MYSQL_USER:-xianyu}
      - MYSQL_PASSWORD=${MYSQL_PASSWORD:-change-me-in-env}
      - TZ=Asia/Shanghai
    command:
    ...
    networks:
      - xianyu-network
    healthcheck:
      test: ["CMD", "mysqladmin", "ping", "-h", "127.0.0.1", "-u", "root", "-p${MYSQL_ROOT_PASSWORD:-change-me-in-env}"]
      interval: 10s
      timeout: 5s
      retries: 10
      start_period: 30s
  ...
  redis:
    ...
    command: >
      redis-server
      --requirepass ${REDIS_PASSWORD:-change-me-in-env}
      --maxmemory 256mb
    ...
    networks:
      - xianyu-network
    healthcheck:
      test: ["CMD", "redis-cli", "-a", "${REDIS_PASSWORD:-change-me-in-env}", "ping"]
      interval: 10s
      timeout: 5s
      retries: 5
      start_period: 10s
  ...
  backend-web:
    ...
    environment:
      ...
      - REDIS_HOST=redis
      - REDIS_PORT=6379
      - REDIS_PASSWORD=${REDIS_PASSWORD:-change-me-in-env}
      - REDIS_DB=${REDIS_DB:-0}
      - BACKEND_WEB_PORT=8089
      - HOST=0.0.0.0
      - JWT_ALGORITHM=HS256
