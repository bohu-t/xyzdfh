...
class AutoDeliveryHandler:
    ...
    async def _auto_delivery(self, item_id: str, item_title: str = None, order_id: str = None, send_user_id: str = None, chat_id: str = None, send_user_name: str = None, skip_confirm: bool = False):
        ...
        try:
            ...
            # 只要有订单ID，就优先尝试读取/补全订单规格。
            # 现场故障：商品详情缓存 multi_spec=False，但商品实际是 SKU 商品，且关联了多张多规格卡券；
            # 如果不取订单规格，会以 spec_name/spec_value=None 匹配，所有多规格卡券都会被过滤掉。
            if order_id:
                logger.info(f"尝试获取订单规格信息: {order_id} (商品多规格状态={is_multi_spec})")
                try:
                    existing_order = db_manager.get_order_by_id(order_id)
                    if existing_order:
                        spec_name = existing_order.get('spec_name') or None
                        spec_value = existing_order.get('spec_value') or None
                        if spec_name and spec_value:
                            logger.info(f"从已入库订单获取到规格信息: {spec_name} = {spec_value}")

                    if not (spec_name and spec_value):
                        order_detail = await self.fetch_order_detail_info(order_id, item_id, send_user_id)
                        if order_detail and isinstance(order_detail, dict):
                            spec_name = order_detail.get('spec_name') or None
                            spec_value = order_detail.get('spec_value') or None
                            if spec_name and spec_value:
                                logger.info(f"从订单详情获取到规格信息: {spec_name} = {spec_value}")
                            else:
                                logger.warning(f"未能从订单详情获取到规格信息")
                        else:
