...
注意：结合商品信息，给出实用建议。"""
    ...
    def _ensure_smart_service_prefix(self, reply: Any) -> str:
        """强制 AI 最终回复使用固定两行格式：第一行【智能客服】，后面是正文。"""
        text = self._normalize_text(reply)
        if not text:
            return ""
        ...
        for raw_line in text.splitlines() or [text]:
            ...
            # 去掉历史同一行前缀，统一改为第一行标题。
            if line == header or line == legacy_prefix:
                continue
            if line.startswith(header):
                line = line[len(header):].lstrip('：: ｜|-— ')
            elif line.startswith(legacy_prefix):
                line = line[len(legacy_prefix):].lstrip('：: ｜|-— ')
    ...
    async def _get_global_ai_defaults(self, db_session: AsyncSession) -> Dict[str, Any]:
        try:
            stmt = select(SystemSetting.value).where(SystemSetting.key == "ai.reply.default_settings")
            result = await db_session.execute(stmt)
            ...
            if not isinstance(parsed, dict):
                return {}
            parsed.pop("ai_enabled", None)
            parsed.pop("enabled", None)
            return parsed
        except Exception as e:
            logger.warning(f"读取全局AI默认配置失败，使用账号配置: {e}")
            return {}

    async def _build_effective_ai_settings(self, account: XYAccount, db_session: AsyncSession) -> Dict[str, Any]:
        ai_settings = (account.metadata_json or {}).get("ai_reply_settings") or {}
        defaults = await self._get_global_ai_defaults(db_session)
        merged = self._get_default_settings()
        merged.update({k: v for k, v in defaults.items() if v is not None})
        merged.update({k: v for k, v in ai_settings.items() if v is not None})
        return self._extract_ai_settings(merged)
    ...
    async def is_ai_enabled(self, cookie_id: str, db_session: AsyncSession) -> bool:
        ...
        try:
            ...
            # 合并全局默认AI配置与账号开关/覆盖项
            settings = await self._build_effective_ai_settings(account, db_session)
    ...
    async def get_ai_settings(self, cookie_id: str, db_session: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            return await self._build_effective_ai_settings(account, db_session)
        except Exception as e:
...
请根据以上信息生成回复："""
                ...
                if reply:
                    reply = self._ensure_smart_service_prefix(reply)
                    if not reply:
                        logger.warning(f"【{cookie_id}】AI回复前缀格式化后为空，跳过发送")
                        return None
