...
class CookieTokenManager:
        ...
        try:
            ...
            if hasattr(self.parent, 'cookie_id') and self.cookie_id:
                try:
                    ...
                    # 保护手动登录/滑块通过后得到的关键风控 Cookie。
                    # 有些接口刷新只返回普通 Cookie，不返回 x5sec；如果直接整串覆盖，会把
                    # 用户刚提供的可用 x5sec 冲掉，下一次 token 刷新又进入 FAIL_SYS_USER_VALIDATE。
                    db_account = await get_account_by_identity(
                        self.cookie_id,
                        owner_id=current_user_id,
                    )
                    if db_account and db_account.cookie:
                        try:
                            current_db_cookies = trans_cookies(db_account.cookie)
                        except Exception:
                            current_db_cookies = {}
                        for protected_key in ("x5sec", "x5secdata", "x5sectag"):
                            old_value = current_db_cookies.get(protected_key)
                            if old_value and not cookies_dict.get(protected_key):
                                cookies_dict[protected_key] = old_value
                                logger.info(f"【{self.cookie_id}】保留数据库中的 {protected_key}，避免无风控字段刷新覆盖")
                        self.cookies_str = '; '.join([f"{k}={v}" for k, v in cookies_dict.items()])
                        self.cookies = cookies_dict

                    logger.info(
                        f"【{self.cookie_id}】update_config_cookies准备保存: "
                        f"cookie_count={len(cookies_dict)}, has_x5sec={bool(cookies_dict.get('x5sec'))}"
                    )
    ...
    async def _mark_account_invalid_once(self, reason: str, notification_type: str = "account_invalid") -> None:
        """账号登录/Token 刷新失败后只标记失效并通知，不继续撞风控。"""
        try:
            from common.db.compat import db_manager
            db_manager.disable_account(self.cookie_id, reason=reason)
        except Exception as e:
            logger.error(f"【{self.cookie_id}】标记账号失效失败: {self._safe_str(e)}")

        try:
            await self._delete_cached_token()
        except Exception as e:
            logger.warning(f"【{self.cookie_id}】清理Token缓存失败: {self._safe_str(e)}")

        self.current_token = None
        self.last_token_refresh_status = "account_invalid"
        await self.send_token_refresh_notification(reason, notification_type)
        ...
        try:
            ...
            try:
                ...
                success, cookies, captcha_engine = await run_browser_task(
                    ...
                    False,  # headless（主引擎）
                    max(20, min(int(os.getenv('CAPTCHA_PLAYWRIGHT_TIMEOUT', '60')), 120)),  # browser_timeout（主引擎）
                ...
                if success and cookies:
                    ...
                    # 合并滑块浏览器返回的全部有效 Cookie，而不是只合并 x5/x5sec。
                    # 闲鱼风控放行通常依赖整组浏览器态 Cookie；只保存 x5sectag 会导致
                    # token 接口继续返回 FAIL_SYS_USER_VALIDATE，账号任务一直 disconnected。
                    ...
                    logger.info(
                        f"【{self.cookie_id}】滑块返回{len(cookies)}个cookies，"
                        f"其中{len(x5sec_cookies)}个x5相关；"
                        f"本次合并新增{new_cookie_count}个、更新{updated_cookie_count}个"
                    )

                    if not x5sec_cookies:
                        logger.warning(
                            f"【{self.cookie_id}】滑块未返回标准 x5sec cookie，"
                            f"仍先合并浏览器返回的全部Cookie后重试Token刷新"
                        )

                    if new_cookie_count == 0 and updated_cookie_count == 0:
                        logger.warning(
                            f"【{self.cookie_id}】滑块返回的Cookie与当前Cookie完全一致，"
                            f"判定为未产生有效更新，避免重复风控循环"
                        )
                        return None
                    ...
                    try:
                        ...
                        logger.info(f"【{self.cookie_id}】滑块验证成功: 新增{new_cookie_count}个Cookie, 更新{updated_cookie_count}个Cookie")
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    # 检查是否需要滑块验证/风控。
                    # 账号一旦撞风控，不再自动过滑块或递归重试，避免越刷越严重。
                    if self.need_captcha_verification(res_json):
                        reason = "账号已失效：Token刷新触发闲鱼风控/滑块验证，请在账号管理中重新手动输入有效Cookie"
                        logger.warning(f"【{self.cookie_id}】{reason}，本次只请求一次，不继续重试")
                        notification_sent = True
                        await self._mark_account_invalid_once(reason, "account_invalid")
                        return None
                    ...
                    reason = "账号已失效：Token刷新失败，请在账号管理中重新手动输入有效Cookie"
