...
class TokenManager:
    ...
    def __init__(self, xianyu_instance):
        ...
        # Cookie刷新配置
        # 旧默认值为 180 秒，容易在账号已触发闲鱼风控后持续撞 token 接口。
        # 默认改为 6 小时，并允许通过环境变量覆盖；设置 COOKIE_REFRESH_ENABLED=false 可关闭。
        self.cookie_refresh_interval = max(0, _env_int('COOKIE_REFRESH_INTERVAL', 21600))
        self.last_cookie_refresh_time = 0
        self.cookie_refresh_lock = asyncio.Lock()
        self.cookie_refresh_enabled = os.getenv('COOKIE_REFRESH_ENABLED', 'true').strip().lower() not in ('0', 'false', 'no', 'off')
        ...
        async with self.cookie_refresh_lock:
            try:
                ...
                if new_token:
                    ...
                    if refresh_status in no_immediate_retry_statuses:
                        logger.warning(
                            f"【{self.cookie_id}】Cookie刷新失败且状态为 {refresh_status}，"
                            "跳过5秒立即重试，避免继续触发闲鱼风控"
                        )
