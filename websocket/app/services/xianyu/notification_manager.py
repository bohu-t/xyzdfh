    ...
    async def _send_to_channels(self, notifications: list, message: str, attachment_path: str = None) -> bool:
        ...
        for notification in notifications:
            ...
            try:
                ...
                safe_keys = sorted(config_data.keys()) if isinstance(config_data, dict) else []
                logger.info(
                    f"📱 通知渠道: {channel_type}, "
                    f"config_keys={safe_keys}, has_webhook={bool(config_data.get('webhook_url') or config_data.get('config')) if isinstance(config_data, dict) else False}, "
                    f"has_secret={bool(config_data.get('secret')) if isinstance(config_data, dict) else False}"
                )
